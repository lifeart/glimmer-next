...
/**
 * Convert Glimmer AST loc (line/column) to character offset range.
 * The template string is needed to calculate offsets from line/column.
 */
function astLocToSourceRange(loc: ASTv1.SourceLocation | null | undefined, template: string): SourceRange | undefined {
  ...
  let startOffset = 0;
  for (let i = 0; i < loc.start.line - 1 && i < lines.length; i++) {
    startOffset += lines[i].length + 1; // +1 for newline
  }
  startOffset += loc.start.column;

  // Convert end position
  let endOffset = 0;
  for (let i = 0; i < loc.end.line - 1 && i < lines.length; i++) {
    endOffset += lines[i].length + 1;
  }
  endOffset += loc.end.column;
...
export function convert(
  ...
  /** Template source string for computing source locations */
    ...
    if (node.type === 'StringLiteral') {
      ...
      if (node.path.original === 'yield' || node.path.original === 'outlet') {
        ...
        return {
          ...
          condition: '',
          isSync: true,
          loc: templateSource ? astLocToSourceRange(node.loc, templateSource) : undefined,
  ...
  function ElementToNode(element: ASTv1.ElementNode): HBSNode {
    ...
    const node = {
      ...
      children: children,
      // Add source location for source mapping
      loc: templateSource ? astLocToSourceRange(element.loc, templateSource) : undefined,
    };
