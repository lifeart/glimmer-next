...
import { isDestroyed, registerDestructor } from './glimmer/destroyable';
...
export function lazy<T>(factory: () => Promise<{ default: T }>): T {
  class LazyComponent extends Component {
    ...
    constructor(params: Record<string, unknown>) {
      ...
      // Register the loading promise with the destructor tree
      // so destruction waits for pending imports to resolve.
      // Note: load() catches all errors internally, but we add .catch()
      // for safety to ensure destruction never fails due to this destructor.
      registerDestructor(this, () => this.loadingPromise.catch(() => {}));
    }
    ...
    _template() {
      ...
      if (!this.suspenseStarted) {
        ...
        this.suspenseContext = getContext<SuspenseContext>(this, SUSPENSE_CONTEXT, false);
        queueMicrotask(() => {
  ...
  implements SuspenseContext
{
  ...
  isReleasedCell: Cell<boolean> = cell(false);
  ...
  get isReleased(): boolean {
    return this.isReleasedCell.value;
  }
  ...
  start(): void {
    ...
    if (newValue === 0) {
      this.isReleasedCell.update(true);
    }
  }
    ...
    return $_fin(
      [
        $_if(
          () => this.isReleased,
          () => {
            // TRUE branch: loading is complete, show the pre-rendered content
            return {
              [RENDERED_NODES_PROPERTY]: Array.from(fragment.childNodes),
            } as unknown as ComponentReturnType;
          },
          (c: IfCondition) => {
            // FALSE branch: not yet released, pre-render content and show fallback
            if (trueBranch === null) {
              trueBranch = $_ucw(
                (ctx) => {
                  return (
