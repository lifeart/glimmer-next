...
export function TresCanvas(this: Component<TresCanvasProps>) {
  ...
  const setupRenderer = () => {
    ...
    if (isDestroyed) return; // Component was destroyed before setup ran
    ...
    try {
      ...
      const animate = (time: number) => {
        if (isDestroyed || !contextState.renderer.value) return;
  ...
  // Track if window resize fallback was used
  ...
  const cleanup = () => {
    ...
    if (contextState.renderer.value) {
      ...
      contextState.renderer.update(null);
    }
    if (statsElement) {
      statsElement.remove();
      statsElement = null;
  ...
  if (typeof window !== 'undefined') {
    setupFrameId = requestAnimationFrame(() => {
      setupFrameId = null; // Clear the ID since the frame has executed
      setupRenderer();

      // Don't set up listeners if already destroyed
