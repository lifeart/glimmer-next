...
import { $_debug_args, CHILD, COMPONENT_ID_PROPERTY, getBounds, isArray, PARENT, RENDERED_NODES_PROPERTY, TREE, type ComponentLike } from '../shared';
...
const dataAdapter = {
  ...
  getRecordFilterValues(_item: Cell | MergedCell) {
    return {
      isNew: true,
      isModified: true,
      isClean: true,
    ...
    get(_, key) {
      ...
      if (key === 'Component') {
        ...
        function componentToRenderTree(component: Component<any> | ComponentLike): any {
          ...
          component[RENDERED_NODES_PROPERTY].forEach((node, _index) => {
            // @ts-expect-error
        ...
        return {
          ...
          Promise: Promise,
          configure(_name: string, _fn: () => void) {
...
(function () {
  ...
  requireModule = function (name: string) {
    if (name === '@ember/object/computed') {
      ...
      return {
        tagForProperty(obj: Record<string, unknown>, _key: string) {
