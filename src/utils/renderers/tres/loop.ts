...
function createPriorityEventHook(debueName: string) {
    let ons = [];
    return {
        trigger(p) {
            console.log(debueName + ':createPriorityEventHook:trigger', ...arguments);
            debugger;
            ons.forEach((el) => el(p));
        },
        dispose() {
            console.log(debueName + ':createPriorityEventHook:dispose', ...arguments);
        },
        on(a: any) {
            ons.push(a);
            console.log(debueName + ':createPriorityEventHook:on', a);
        }
    }
}
...
export function createRenderLoop(): RendererLoop {
  ...
  const subscribersBefore = createPriorityEventHook<LoopCallbackWithCtx>('subscribersBefore')
  const subscriberRender = createPriorityEventHook<LoopCallbackWithCtx>('subscriberRender')
  const subscribersAfter = createPriorityEventHook<LoopCallbackWithCtx>('subscribersAfter')
