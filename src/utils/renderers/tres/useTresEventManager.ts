...
export function useTresEventManager(
  ...
  const objectsWithEvents = shallowRef((scene?.children as TresInstance[]).filter(hasChildrenWithEvents) || [])
  ...
  function registerObject(maybeTresObject: unknown) {
    if (is.tresObject(maybeTresObject) && is.object3D(maybeTresObject)) {
      objectsWithEvents.push(maybeTresObject as TresInstance)
    }
  }

  function deregisterObject(maybeTresObject: unknown) {
    if (is.tresObject(maybeTresObject) && is.object3D(maybeTresObject)) {
      const index = objectsWithEvents.indexOf(maybeTresObject as TresInstance)
      if (index > -1) {
        objectsWithEvents.splice(index, 1)
      }
    }
  }
